...
export default function BottomNav() {
  return (
    <nav className="fixed inset-x-3 bottom-3 z-30 rounded-2xl border border-white/80 bg-white/95 shadow-float backdrop-blur-xl md:hidden">
      <div className="mx-auto flex max-w-screen-sm items-center justify-around px-2 py-1.5">
        {items.map((item) => (
          <NavLink key={item.to} to={item.to} className="w-14 text-xs font-medium text-stone-600" end={item.to === '/'}>
            {({ isActive }) => (
